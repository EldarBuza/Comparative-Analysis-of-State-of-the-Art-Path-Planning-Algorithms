...
double AStar::calculateHeuristic(const Node& a, const Node& b) {
    // Euclidean distance
...
PlannerResult AStar::plan(const Grid& grid, const Node& start, const Node& goal) {
    Metrics metrics;
    metrics.plannerName = "A*";
    ...
    while (!openSet.empty()) {
        ...
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                ...
                if (grid.isObstacle(neighborX, neighborY)) continue;
